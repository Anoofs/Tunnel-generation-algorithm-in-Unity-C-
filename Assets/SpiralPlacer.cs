...
public class SpiralPlacer : PipeItemGenerator {
    ...
    {
        float start = (Random.Range(pipe.pipeSegmentCount - (pipe.pipeSegmentCount * pipe.DifficultyFactor), pipe.pipeSegmentCount) + 0.5f);
        float direction = Random.value < 0.5f ? 1f : -1f;

        float angleStep = pipe.CurveAngle / pipe.CurveSegmentCount;
        for (int i = 0; i < (pipe.CurveSegmentCount * pipe.DifficultyFactor); i++)
        {
